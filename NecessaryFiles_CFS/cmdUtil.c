...
Function: This is an adapted version of original CFS tool cmdUtil. This version compute 
a checksum of the packet and add the value as a 4 bytes string at the end of the packet.
*****************************************************************************************
***********************************************************************/
...
unsigned char csum(unsigned char *ptr, size_t sz){
	unsigned char chk = 0;
	while (sz-- != 0)
		chk -= *ptr++;
	return chk;
...
int main(int argc, char *argv[]) {
    ...
    */	
    ...
    int i = 0;
    size_t packetSize = CommandData.DataLen+8;
    char packetData[packetSize*4];
    char temp1[5];
    while (i < packetSize) {
        snprintf(temp1, packetSize,"0x%02X", CommandData.PacketHdr[i] & 0xFF);
        strcat(packetData, temp1);
	i++;
    }
    printf("packetData: %s\npacketSize: %d\n",packetData,packetSize);

    unsigned char checksum = csum (packetData, packetSize);
    printf("checksum: %c",checksum);
    char buffer[4];
    itoa(checksum, buffer,10);
    ProcessLongArgument(buffer, &CommandData);
